...
export function CardStack<T extends CardStackItem>({
  ...
  React.useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      setActive((prev) => wrapIndex(prev + 1, len));
    }, Math.max(700, intervalMs));

    return () => window.clearInterval(interval);
  }, [autoAdvance, intervalMs, len, pauseOnHover, hovering]);
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-3xl overflow-hidden shadow-xl",
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          {activeItem.href ? (
            <Link
              ...
              target="_blank"
              rel="noreferrer"
              className="text-berry hover:text-wine transition"
