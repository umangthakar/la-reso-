...
import { Star, ShoppingCart, Zap, ArrowRight, MessageCircle } from "lucide-react";
...
/* ------------------------------------------------------------------ *
 * Le Rasa â€” Animated Product Card
 *
 * A three-step reveal built on Framer Motion, triggered on hover
 * (desktop) or tap (touch):
 *
 *   STEP 1 â€” Initial : minimal blush trapezoid container, image tucked
 *                      to the right, only title + price on the left.
 *   STEP 2 â€” Slide-in: container elevates & scales; a white details
 *                      panel slides in from the left (half-overhanging
 *                      the container) with a staggered content reveal â€”
 *                      Title â†’ Rating â†’ Price â†’ Description â†’ Add to Cart.
 *   STEP 3 â€” Focus   : the product image zooms & takes the right half,
 *                      the backdrop dims, and a price badge, star rating,
 *                      ADD TO CART + BUY NOW surface over the image.
 * ------------------------------------------------------------------ */

// Every product shows a static five-star display (visual only â€” no review
...
const detailsPanel: Variants = {
  // Slides in from the left (Blueprint â€” Slide In: 0.6s)
...
export function AnimatedProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      {/* Blush trapezoid backdrop â€” right edge sits higher than the left.
      ...
      {/* ------------------------------- LEFT / DETAILS -------------------------------
          ...
          Everything stays inside this column â€” no overhang. */}
      <div className="relative order-2 flex min-w-0 flex-1 flex-col justify-center p-4 md:order-1 md:p-5">
        {/* Desktop resting label â€” cross-fades out on hover */}
        <motion.div
        ...
        {/* Desktop hover panel â€” slides in from the left, fully inside the column */}
        <motion.div
        ...
        {/* Mobile static details â€” always visible, stacked under the image */}
      ...
      <div className="relative order-1 aspect-[4/3] w-full shrink-0 overflow-hidden md:order-2 md:aspect-auto md:h-full md:w-[44%]">
        ...
        {/* Rating + actions â€” bottom of the image */}
          ...
          >
            {isCustomCake ? (
              ...
              <>
                ...
                {/* Buy Now never buys straight from the card â€” it opens the
                    product page, where the size is chosen before customizing. */}
                <Link
                  href={detailHref}
                  onClick={(e) => e.stopPropagation()}
        ...
        {/* Category tag chip â€” resting state accent */}
        {product.tag && (
...
/* Shared details block â€” rendered as a staggered reveal on desktop hover
  ...
  return (
    <>
      ...
      <Row {...rowProps} className="mt-3">
        {/* Read More always opens the full product page (/menu/[slug]). */}
        <Link
          href={detailHref}
          onClick={(e) => e.stopPropagation()}
